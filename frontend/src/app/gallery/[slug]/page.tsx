...
const GalleryComponent = ({ value }: { value: any }): ReactElement | null => {
  ...
  return (
    <div className="gallery">
      {value.images && value.images.map((image: SanityImageSource, index: number) => (
        <Image
          key={index}
...
<div className="mt-48 rounded-md bg-dg-900 overflow-hidden h-96 max-w-6xl m-auto flex justify-center items-center relative">
          <Image
            ...
            alt={title || "blogPost"}
            className="h-[120%] object-cover absolute"
            height={600}
            width={1200}
          />
        </div>
      <div className="p-8 max-w-4xl m-auto">
        <div className="mb-4">
          <Link href="/gallery">← Retour aux galeries photo</Link>
        </div>
        <GalleryComponent value={pageBuilder} />
